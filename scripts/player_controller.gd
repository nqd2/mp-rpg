...
func _ready() -> void:
	...
	var texture := load("res://assets/characters/player.png")
	if texture == null:
		push_error("Missing texture at res://assets/characters/player.png")
		return

	_build_sprite_frames(texture)
...
func _on_animation_finished() -> void:
	if is_attacking and animated_sprite.animation.ends_with("_attack"):
		is_attacking = false
		_refresh_animation_for_move(_get_wasd_dir() != Vector2.ZERO)
